...
1. A *line* is defined as a particle that varies substantially along one dimension, and negligibly along the other two dimensions. We have measured the length and width of a bounding box containing the line. We do not know whether or not the line is coiled within this box. We can state that each line is at least as long as the diagonal of the bounding box that contains it. This uncertainty incurs additional measurement error. We include a dichotomous indicator variable to distinguish lines from other particles. **NOTE: The current data file does not contain true value of width for lines!**
```{r}
# Do this with raw data; current file has imputed value for width
#df$length[df$Line == 1] <- sqrt(df$length[df$Line == 1]^2 + df$width[df$Line == 1]^2)
```
2. A *film* is defined as a particle that varies substantially along two dimensions, and negligibly along the third. We have measured the length and width of these particles, which describes them accurately. We include a dichotomous indicator variable to distinguish films from other particles.
3. All other particles vary substantially along three dimensions, of which we have measured two. Assuming that three-dimensional particles are randomly oriented on the plane, we can assume that the correlation between the first two dimensions is similar in size to the correlation between each dimension and the third.
...
if(!file.exists("res_line.RData")){
  res_line <- estimate_profiles(analyze_line, 1:5, variances = c("varying"), covariances = c("zero"), package = "MplusAutomation", OUTPUT = "svalues;", keepfiles = TRUE)
...
if(!file.exists("res_film.RData")){
  res_film <- estimate_profiles(analyze_film, 1:5, variances = c("varying", "varying"), covariances = c("equal", "varying"), package = "MplusAutomation")
